...
export function Inject(...dependencies: ClassConstructor[]): MethodDecorator & PropertyDecorator {
    ...
    return <MethodDecorator & PropertyDecorator>(
        <T extends { [index: string]: any }>(target: T, propertyKey: keyof T, descriptor?: TypedPropertyDescriptor<T>) => {

            // get the type of the dependency if it is a property decorator
            if (dependencies.length === 0 && !descriptor) {

                // resolve the type
                const token = Reflect.getMetadata("design:type", target, propertyKey as string);
            ...
            if (!descriptor) {
                ...
                target[propertyKey] = (resolvedDependencies.length === 1 ? resolvedDependencies[0] : resolvedDependencies) as any;
